...
async function ruleBasedReply(ctx, intent, message) {
  ...
  if (intent === "how_to") {
    ...
    if (m.includes("checkout")) {
      return `**Checkout:** Add items from product pages → open **Cart** (/cart) → **Checkout** (/checkout). Sign in for faster checkout or continue as guest. Enter shipping, pick payment, and confirm.`;
    }

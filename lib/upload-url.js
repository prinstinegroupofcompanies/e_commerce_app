...
/**
 * Repair external CDN URLs missing a protocol or host prefix.
 * @param {string | null | undefined} url
 */
export function repairExternalMediaUrl(url) {
  if (!url || typeof url !== "string") return url;
...
export function resolveMediaUrl(url) {
  ...
  const external = repairExternalMediaUrl(trimmed);
  if (external !== trimmed) return external;

  if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) return trimmed;

  const normalized = normalizeLegacyUploadPath(trimmed);
  if (/^\/(uploads|products)\//i.test(normalized)) {
    return normalized;
  }

...
import { isBareUploadFilename } from "@/lib/upload-url";
...
/**
 * Locate a stored upload by bare filename (searches uploads/* and legacy public/products).
 * @param {string} filename
 */
export async function findUploadedFile(filename) {
  const safe = path.basename(String(filename || "").trim());
  if (!safe || safe.includes("..") || !isBareUploadFilename(safe)) return null;

  const publicRoot = getPublicRoot();
  const candidates = [
    path.join(publicRoot, "uploads", safe),
  ];
  ...
  try {
    ...
    for (const ent of entries) {
      if (ent.isDirectory()) {
        candidates.push(path.join(uploadRoot, ent.name, safe));
      }
    }
  } catch {
  ...
  candidates.push(path.join(publicRoot, "products", safe));

  for (const absolute of candidates) {
    try {
      await fs.access(absolute);
      const relative = absolute.slice(publicRoot.length).replace(/\\/g, "/");
